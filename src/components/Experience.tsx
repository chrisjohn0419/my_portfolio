...
<div className={`p-3 rounded-lg transition-colors ${darkMode ? 'bg-blue-900/50 group-hover:bg-blue-800/50' : 'bg-blue-50 group-hover:bg-blue-100'}`}>
                  ...
                  <div>
<h3 className={`text-2xl font-bold mb-1 ${darkMode ? 'text-gray-100' : 'text-slate-800'}`}>
                      {exp.role}
                    </h3>
                    <p className="text-lg text-blue-600 font-semibold mb-1">
                      {exp.company}
                    </p>
                    <p className={`${darkMode ? 'text-gray-500' : 'text-slate-500'} text-sm`}>{exp.location}</p>
                  </div>
                </div>

<div className={`flex items-center gap-2 px-4 py-2 rounded-lg ${darkMode ? 'text-gray-400 bg-gray-700' : 'text-slate-600 bg-slate-50'}`}>
                  <Calendar size={18} />
...
<h4 className={`text-sm font-semibold mb-3 uppercase tracking-wide ${darkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                ...
                <ul className="space-y-2">
                  {exp.responsibilities.map((resp, idx) => (
                    <li key={idx} className={`flex items-start gap-2 ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
